...
### Signature evidence and setup health

Valid and rejected webhook signatures are separate evidence streams. `last_signature_verified_at` is the durable readiness milestone for a correctly signed provider event. `last_signature_rejected_at` and `rejected_signature_count` are security telemetry. Every invalid signature still fails closed with HTTP 403 and is recorded, but it does not replace an earlier valid-signature milestone or make an otherwise healthy channel appear disconnected.

Admin displays the last valid signed webhook, last rejected attempt, rejected-attempt count, and last inbound event separately. These timestamps and counts are operational evidence only; internal identifiers, signatures, and payloads remain hidden.

Setup state describes onboarding progress. Connection health describes current operation. A newly completed setup remains `connected` until its health evaluation succeeds, then becomes `healthy`. A previously completed record may recover from stale `requires_attention` when its durable checklist is complete and current connection health is healthy. A historical test-send error does not permanently block that recovery once successful outbound, inbound, and signature milestones satisfy readiness.

Run the guarded negative-signature check with `PHASE_Q_SIGNATURE_NEGATIVE_CONFIRM=YES scripts/phase-q-pilot-verify.sh`. A pass requires all of the following: HTTP 403, an increased rejected-attempt count, preserved `signature_status=verified`, and coherent healthy setup state.
